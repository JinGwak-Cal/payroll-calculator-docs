...
> manual-v14.md 대기 A~F 및 본문 P/UX/UI 항목 전수 재정리.
> 분류 기준: absolute-rules.md 목차08 참조
> 형식: 상태 / 태그 / 배경 / 이용자필요 / 목표 / 처리방침 / 구현순서 / 판정

## 상태 = 완료

### P3 — juhu row 복귀
- 태그: Feature
- 배경: allowanceRows juhu row + JuhuSection 패널 연결 필요성 제기
- 이용자필요: 주휴수당 정보를 다른 수당과 동일한 형식으로 확인
- 목표: juhu row를 ResultGrid에 정상 연결
- 처리방침: 확인 결과 이미 구현됨
- 구현순서: (해당없음 — 기 구현 확인만)
- 판정: 완료
- 원문: manual-v14.md 145행, ResultGrid.tsx:136 / JuhuSection.tsx 연결 완료

### §19 변경분 (대기E)
- 태그: Docs
- 배경: §19 가산율 기반 제수당 UI 목업 스펙 변경분 반영 필요성 제기
- 처리방침: manual-v14.md 1757행 자체에 완료 기록 확인
- 판정: 완료

### PremiumScreen/PremiumSection/CustomPremiumCard/usePremium 정리
- 태그: Feature
- 배경: 가산수당 입력이 다중 화면(PremiumScreen 전체화면)에 분산되어 있음
- 이용자필요: 단순한 단일 입력 경로
- 목표: Drawer 단일 진입점으로 통합
- 처리방침: 흡수 후 삭제(선삭제 금지 원칙 준수)
- 구현순서: STEP6-2 1/5~5/5 단계별 진행, 참조 0건 확인 후 삭제
- 판정: 완료
- 원문: D-05-09, D-05-12

### P8 — premiumHours 역산 Float Drift
- 태그: Bug
- 배경: §6.7 원문 — input(총시간)/safeMult≠mult 오차 발생 가능
- 이용자필요: 입력한 시간이 정확히 계산에 반영
- 목표: allowanceRows 역산 경로의 오차 제거
- 처리방침: mult 역산·보정
- 구현순서: 변환계층(custom-premium.ts)에서 총시간/mult 역산 + 부동소수점 언더슈팅 보정
- 판정: 완료 (premiumHours 역산 경로 한정)
- ※ 앱 전체 Float Drift 검증은 별도 항목(Float Drift 전수 검증)으로 분리
- 원문: manual-v14.md 649행, D-05-10

## 상태 = 완료(대체)

### UX-02 — 펼침 항목 글로우잉 효과
- 태그: UX
- 배경: 펼칠 수 있는 항목임을 시각적으로 인지시키는 목적
- 이용자필요: (구목적) 입력 유도 시각힌트
- 목표: (구목표) 닫힘 상태 시 글로우 펄스
- 처리방침: 재구현 금지, Shimmer가 동일 목적 대체 완료
- 구현순서: (해당없음)
- 판정: 완료(대체)
- 재오픈 조건: 사용자 피드백으로 glow 재요청 시
- 원문: manual-v14.md 28~31행, 869행, 1238행, 1575행

### manual-v14 전체 — 전수조사
- 태그: Docs
- 배경: 대기 A~F 및 본문 산재 항목 추적 누락 반복 방지
- 처리방침: 본 "분류 이력(2606.19)" 섹션 작성으로 1차 목적 달성
- 구현순서: (해당없음)
- 판정: 완료(대체) — 문서체계 정비는 absolute-rules.md 목차08 신설로 승계

## 상태 = 승계

### P7 — 연차수당 신설 구현 (§15)
- 태그: Feature
- 배경: 연차수당이 정확히 계산·표시되어야 함
- 이용자필요: 연차수당 금액·조건을 명확히 확인
- 목표: §15/UI-M 기준 연차수당 신설 구현
- 처리방침: STEP6-1로 승계, manual-v14 추적 종료
- 구현순서: STEP6-1에서 별도 설계
- 판정: 승계 → STEP6-1
- 원문: manual-v14.md 160행 / STEP5-4 §2

## 상태 = 작업확정

### ResultGrid 순서 정렬
- 태그: Design
- 배경: 문서상 확정 순서(주휴→연차→연장→야간→휴일→공제)와 실제 구현 불일치
- 이용자필요: 일관된 정보 배치로 혼동 방지
- 목표: ResultGrid Row 순서를 문서 확정 순서로 정렬
- 처리방침: STEP6-1 첫 작업으로 편입
- 구현순서: 연차 개선 작업 착수 전 Row 순서 정렬 선행
- 판정: 작업확정 [높음] (배치 정렬 — 동작 오류 아님)
- 원문: STEP5-4 §1, manual-v14 §19 1행

### 18-A — 저장기록 카드 2줄 구조
- 태그: Feature
- 배경: 저장기록 목록의 가독성 부족
- 이용자필요: 핵심 정보(금액·수당특징)를 한눈에 확인
- 목표: 1줄 제목·금액 / 2줄 수당요약 고정 표시
- 처리방침: History UX 개선 STEP 신설 시 구현
- 구현순서: 2줄 내용 포맷 확정(선행) → 요약 생성 함수 작성 → 카드 레이아웃 적용
- 판정: 작업확정 [보통]
- 원문: manual-v14.md 994~997행

### 18-B — 드래그 리오더링
- 태그: Feature
- 배경: 사용자별 조회 우선순위가 상이함
- 이용자필요: 카드 순서·항목 순서를 직접 조정
- 목표: (1) 7항목 표시순서 (2) 카드 자체 순서, 분리 구현
- 처리방침: History UX 개선 STEP에서 구현
- 구현순서: 카드순서 reorder 먼저 → 항목순서 전역설정 별도 키로 전체카드 공통 적용
- 판정: 작업확정 [보통]
- 원문: manual-v14.md 999~1006행

### 18-C — 근무지 합산
- 태그: Feature
- 배경: 복수 알바(근무지) 이용자의 총소득 확인 니즈
- 이용자필요: 여러 근무지 합산 총액을 한 화면에서 확인
- 목표: 다중선택 → 계산결과와 동일 형식으로 합산값 표시
- 처리방침: 독립 STEP 신설, STEP6 계열과 분리 (규모 큼)
- 구현순서: [선택]버튼→체크박스 노출 → 선택기록 합산로직 → 제목 자동조합("합산-A+B") → 기존 계산결과 포맷 재사용
- 판정: 작업확정 [보통]
- 원문: manual-v14.md 1009~1020행

### P-C — 가산율 비정수 입력 (0.3, 0.8 등)
- 태그: Feature
- 배경: 특수 가산율(소수점) 입력 필요한 현장 존재 가능성(W-10 기준)
- 처리방침: 실사용 사례 조사 후 필요성 확인하여 구현
- 판정: 작업확정 [낮음]

### UX-01 — 세금·공제 4지선다 아코디언
- 태그: Design
- 배경: 4개 옵션 동시노출로 시각적 복잡
- 이용자필요: 선택한 항목만 보고 싶음
- 목표: 선택 시 해당 항목만 펼침, 나머지 자동 축소
- 처리방침: UI 개선 과제로 승격, STEP6-1 이후 위치
- 구현순서: 대상파일(OptionsSection.tsx 또는 SupplementScreen.tsx) 확인 선행
- 판정: 작업확정 [STEP6-1 이후]

### 대기#5 — 표준 진입 프롬프트 중복 보관
- 태그: Docs
- 배경: 매 세션마다 영향을 주는 운영 인프라, GPT 진입 실패 시 백업 경로 필요
- 처리방침: absolute-rules.md / index.md 양쪽 보관
- 구현순서: index.md 목차02 원문을 absolute-rules.md 목차01에 그대로 복사
- 판정: 작업확정 [높음]

### UI-Audit 기반 UI 통일 적용
- 태그: Design, UX
- 배경: UI-Audit(01~06) 및 UI 인벤토리를 통해 색상/강조/컴포넌트/레이어 기준이 확정됨
- 이용자필요: 더 깔끔하고 일관된 UI
- 목표: Home / ResultGrid / 상태유급형 / 시간속성 수당 UI를 통일된 디자인 언어로 정비
- 처리방침: UI-Audit 결과를 기준으로 단계별 적용
- 구현순서:
  1. STEP6-1 연차
  2. ResultGrid
  3. 상태유급형
...
### 모드 확장 (2×2 구조)
- 태그: Design
- 배경: 현재 uniform/timerange 중심 구조는 요일별 상이 근무시간을 충분히 표현하지 못함
- 이용자필요: 실제 근무표에 가까운 입력
- 목표: 2×2 확장모드 도입 여부 및 구조 확정
- 처리방침: 현재 STEP 완료 후 별도 설계 검토 (Jin님 주도)
- 구현순서:
  1. 요구사항 정리
  2. 입력구조 설계
  3. 계산엔진 영향 분석
...
### BUG-01 — 기록보기 저장버튼 토글
- 태그: Bug
- 배경: 저장버튼이 저장/취소 토글로 동작해야 하나, 누를 때마다 계속 저장만 되는 문제
- 이용자필요: 저장 상태를 명확히 제어
- 목표: 저장↔취소 토글 정상화
- 처리방침: 실제 현 코드 동작 확인 후 버그 수정 여부 결정
- 구현순서: History 저장 상태 판정 로직 확인 → 버튼 상태/동작 수정
- 판정: 검토대기
- 원문: manual-v14.md 178~180행

### P9 — PremiumRateCard/NightPayCard
- 태그: TechDebt
- 배경: 문서 내부 모순(1051행 "P9 완료로 해소" vs 1752행 "재작업 필요")
- 처리방침: 실제 코드 존재·참조 확인 후 삭제/유지 판정
- 판정: 검토대기

### P11 — Status 타입 확장 (fail_hours/fail_absent)
- 태그: Design
- 배경: 주휴수당 미발생 사유(시간부족 vs 결근) 세분화 필요성 제기
- 이용자필요: 미발생 사유를 명확히 안내받고 싶음
- 처리방침: WeekStatus 코드 실존 여부 확인 후 유지/폐기 결정
- 판정: 검토대기 (절대 임의 폐기 금지)
...
### UI-L — 라벨 클릭→정보 표시
- 태그: KnownIssue
- 배경: 적용 화면(기록보기 vs 수당화면) 미정
- 처리방침: 적용 대상 화면 확정 후 설계
- 판정: 검토대기 (Known Issue #1)

### Fast Refresh — use-calc.tsx
- 태그: TechDebt
- 배경: Provider+Hook 동일파일 export로 dev HMR 간헐 invalidate
- 처리방침: 기록만, 수정은 별도 Task
- 판정: 검토대기

### Float Drift 전수 검증
- 태그: Verification
- 배경: D-05-10 구현 과정에서 allowanceRows 역산 경로의 부동소수점 언더슈팅
  (1원 손실, 사례: 야간 30h/근무 22일 → 154,800원이 154,799원으로 언더슈팅)이
  발견되었고, 해당 경로는 보정 완료 및 재검증 완료(D-05-10/P8).
- 이용자필요: (해당없음 — 이미 해결된 구간)
- 목표: (해당없음 — 신규 기능 아님)
- 처리방침: 앱 전체 계산 경로(주휴/연차/세금/보험/근무지합산 등)에 대한 float
  drift 전수 검증은 아직 수행되지 않음. 단, 현재 알려진 버그는 없음.
  별도 검증 작업 착수 시 경계값 테스트 수행.
- 구현순서: STEP6 종료 후 계산 경로별(주휴/연차/세금/보험/가산수당) 경계값 테스트
- 판정: 검토대기 — 누락방지용 기록 (정식 작업확정 아님, 현재 우선순위 낮음)
- 원문: D-05-10, P8

### P-A — 기록보기 버튼
- 태그: Feature
- 배경: 기록보기 진입경로 가시성 문제 제기(W-02/W-05 기준)
- 처리방침: 현재 구현 대조 필요 — 실존 여부 미확인
- 판정: 검토대기

### P-B — 피드백 기능
- 태그: Feature
- 배경: 사용자 의견 수렴 채널 필요(W-16 기준), 구글폼+이메일 테스트 진행 이력 있음
- 처리방침: 현재 구글폼/메일 체계 실제 작동 여부 확인 후 완료/작업확정 판정
- 판정: 검토대기

### P-D — 하단 기능모음
- 태그: Feature
- 배경: 계산결과 공유·내보내기 접근성 필요(W-23 기준)
- 처리방침: 현재 구현 대조 필요 — 실존 여부 미확인
- 판정: 검토대기
...
- 처리방침: 폐기 (현 구조와 불일치)
- 판정: 폐기
- 원문: manual-v14.md 156행

### P6 — 검증 체크리스트
- 배경: STEP 3-F(usePremium 추출) 단계별 검증 절차
- 처리방침: 폐기 (usePremium 자체가 STEP6-2 5/5로 제거됨)
- 판정: 폐기
- 원문: manual-v14.md 159행, 1138행

### §19 [C][D][E] — 단일/이중/삼중 PremiumCard
- 처리방침: 폐기 (Drawer 구조가 완전 대체)
- 판정: 폐기
- 원문: §19 [C][D][E] 절
...
* 상태: 완료
* 태그: Docs
* 배경: 대형 파일 교체 및 전체 출력 과정에서 불필요한 토큰 사용 반복
* 목표: 작업 전 토큰 절약 검토 절차 정립
* 처리방침: absolute-rules.md R7~R14로 승계
* 판정: 완료(대체)

### 토큰 절약 검토 절차 (R15~R17) 도입

* 상태: 완료
* 태그: Docs
* 배경: R7~R14 도입 후에도 검토 없이 300줄 이상 파일 전체 전달 및 프롬프트 전체 재출력 반복 발생
* 목표: 작업 전 예상 성과 + 작업 후 실제 성과 분리 보고, 프롬프트 변경분만 제시
* 처리방침: decisions D-05-13 + absolute-rules R15~R17으로 확정
* 판정: 완료(대체)
...
* 상태: 검토대기
* 태그: Feature
* 배경: 급여명세서 OCR → 근로계약서 OCR → N잡러 기능으로 이어지는 사용자 유입 경로 가능성 검토
* 처리방침: 근무지 합산(18-C) 및 핵심 계산 기능 안정화 이후 검토
* 판정: 검토대기
