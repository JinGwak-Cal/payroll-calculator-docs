...
## Rule 17 : Patch-Freeze Applicability Rule

Patch-Freeze 프로토콜은 문서 성숙도(Level)에 따라 적용 강도가 다르다.

| Level | 상태 | 적용 규칙 |
|---|---|---|
| PF Level 0 | Draft | 전체 재작성 허용 |
| PF Level 1 | Review | Patch 권장 |
| PF Level 2 | Freeze Candidate | Patch 필수 |
| PF Level 3 | Frozen | Patch만 허용 |

**Origin:** Sprint-1에서 Draft 단계 문서를 Freeze 단계와 동일하게 취급해 불필요한 전체 재작성이 발생한 사례에서 도출. Draft와 Frozen을 구분하지 않으면 AI가 초안 단계에서 위축되고, Freeze 단계에서 변경 추적성이 무너진다.

**이번 사례 처리:** TCA v1.0→v1.1 전체 재작성은 Draft 단계이므로 위반 아님. Engineering Change Registry에 위반으로 기록하지 않는다. Sprint-1 Lessons Learned에만 기록.
