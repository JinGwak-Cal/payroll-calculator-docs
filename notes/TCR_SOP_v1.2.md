...
Proposal**을 제출한다. Change Proposal에는 다음 7개 항목을
포함한다(v1.2 4차 확정 — 삭제/신규추가 분리):
1. 변경 목적
2. 변경 범위
3. 유지되는 항목
4. **삭제되는 항목**
...
> **역할 분리 원칙(v1.2 4차 확정)**: AI는 Change Proposal을
> 제출할 책임이 있으며, 변경의 적절성·완전성에 대한 최종 판단은
> 사용자에게 있다.
...
## 변경 이력
- **TCR_SOP_v1.2 (2026-07-22, 4차 확정)**: Change Proposal을
  5필드→7필드로 확장(삭제되는 항목/신규 추가되는 항목을 "변경
  또는 삭제"에서 분리) — 삭제 항목이 뭉개져 안 보이는 위험 방지.
  역할분리 원칙 한 줄 추가("AI는 제출 책임, 최종판단은 사용자")
